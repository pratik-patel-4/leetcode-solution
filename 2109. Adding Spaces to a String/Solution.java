public class Solution {
    ...
    public static String addSpaces(String s, int[] spaces) {
        ...
        char[] returnChar = new char[s.length() + spaces.length];
        int idx = 0, c = 0;
        for(int sp : spaces) {
            while (c < sp) {
                returnChar[idx] = ch[c];
    ...
    public static String addSpacesWithSubSeq(String s, int[] spaces) {
        ...
        while(i <= spaces.length) {
            if(i == spaces.length) {
                sb.append(s.subSequence(prev, s.length()));
                i++;
            } else {
                sb.append(s.subSequence(prev, spaces[i]));
                sb.append(" ");    
                prev = spaces[i];      
                i++;
            }
